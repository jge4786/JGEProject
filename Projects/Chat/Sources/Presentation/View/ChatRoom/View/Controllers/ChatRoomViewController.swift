...
public class ChatRoomViewController: UIViewController {
    ...
    @IBOutlet weak var inputTextView_: UITextView!
    
    var bodyWrapper = UIView().then {
    ...
    var contentTableView = UITableView().then {
    ...
    var footerButtonStackView = UIStackView().then {
        $0.axis = .horizontal
        $0.distribution = .equalSpacing
        $0.spacing = 1
    }
    
    var footerWrapperView = UIView().then {
        $0
    }
    
    var letterCountWrapperView = UIView().then {
        $0.layoutMargins = UIEdgeInsets(top: 0.0, left: 0.0, bottom: 5.0, right: 10.0)
    }
    
    var letterCountLabel = PaddingLabel().then {
        $0.textColor = .white
        $0.topInset = 3
        $0.bottomInset = 3
        $0.leftInset = 7
    ...
    var scrollToBottomButton = UIButton().then {
        $0.alpha = 0.85
        $0.contentVerticalAlignment = .bottom
        $0.setImage(UIImage(systemName: "chevron.down.circle.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 45)), for: .normal)
        $0.setTitle("", for: .normal)
        $0.contentEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 15, right: 15)
    }
    
    var addImageButton = UIButton().then {
        $0.setTitle("", for: .normal)
        $0.setImage(UIImage(systemName: "plus"), for: .normal)
        $0.contentEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 10)
    }
    
    var inputTextViewWrapper = UIView().then {
        $0.backgroundColor = Color.Black
        $0.clipsToBounds = true
        $0.layer.cornerRadius = 15
        $0.layer.borderWidth = 1
        $0.layer.borderColor = Color.DarkerGray.cgColor
    }
...
//    @IBOutlet weak var inputTextViewHeight: NSLayoutConstraint!
    @IBOutlet weak var contentWrapperView_: UIView!
...
//        : sendMessage(owner: userData, text: inputTextView.text)
    ...
    var navBar = UINavigationItem().then {
        $0.title = "채팅방이름"
    }
    
    @IBOutlet weak var goBackButton_: UIBarButtonItem!
    ...
    var goBackButton = UIBarButtonItem().then {
        $0.image = UIImage(systemName: "chevron.left")
        $0.tintColor = .white
    }
//    var goBackButton = UIButton().then {
//        $0.tintColor = .black
...
//    }
    ...
    //채팅방 기본 정보
    var chatRoomInfo: (userId: Int, roomId: Int) = (userId: 5, roomId: 0)   // ChatRoomListViewController 에서 넘기는 값을 저장
